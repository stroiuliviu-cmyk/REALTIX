...
const DARK: Palette = {
  pageBg: '#0a0f1d', bg: '#0b1120', raised: '#141d33', raised2: '#0f1828',
  text: '#f1f5f9', text2: '#aab6c8', muted: '#64748b', border: 'rgba(255,255,255,0.10)', border2: 'rgba(255,255,255,0.18)',
  primary: '#3b82f6', primaryH: '#2563eb', primarySoft: 'rgba(59,130,246,0.16)',
  userBub: '#2563eb', asstBub: '#141d33', header: 'rgba(11,17,32,0.82)', shadow: '0 1px 3px rgba(0,0,0,0.45)',
  extBg: 'rgba(129,140,248,0.18)', extText: '#a5b4fc', danger: '#f87171', dangerBg: 'rgba(248,113,113,0.12)', dangerBd: 'rgba(248,113,113,0.3)',
};
...
function formatPrice(card: ListingCard, lang: Language): string {
  ...
  const base = card.currency === 'EUR' ? `€${amount}` : `${amount} lei`;
...
function Header({ ctx }: { ctx: Ctx }) {
  ...
  const langBtn = (code: Language) => (
    <button
      ...
      style={{ padding: '5px 9px', border: 'none', borderRadius: 7, fontSize: 12, fontWeight: 700, cursor: 'pointer', background: lang === code ? p.primary : 'transparent', color: lang === code ? '#fff' : p.text2 }}
    >
      {code.toUpperCase()}
    </button>
  );
    ...
    <>
      ...
      {m.listings && m.listings.length === 0 && !m.error ? (
        <div style={{ marginTop: 11, background: p.asstBub, border: `1px solid ${p.border}`, borderRadius: 14, padding: '15px 16px', maxWidth: 400 }}>
          ...
          <button onClick={() => sendText(t('btn.relax_filters'))} style={{ background: p.primary, color: '#fff', border: 'none', padding: '9px 16px', borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>{t('btn.relax_filters')}</button>
        </div>
      ) : null}
      ...
      {m.error === 'server_error' ? (
        <div style={{ marginTop: 11, display: 'flex', alignItems: 'center', gap: 11, background: p.dangerBg, border: `1px solid ${p.dangerBd}`, borderRadius: 14, padding: '13px 15px', maxWidth: 400 }}>
          ...
          <button onClick={() => sendText(ctx.draft || t('btn.retry'))} style={{ flex: 'none', background: p.primary, color: '#fff', border: 'none', padding: '8px 14px', borderRadius: 9, fontSize: 12.5, fontWeight: 600, cursor: 'pointer' }}>{t('btn.retry')}</button>
        </div>
      ) : null}
      ...
      {m.error === 'rate_limited' ? (
        <div style={{ marginTop: 11, background: p.primarySoft, border: `1px solid ${p.border}`, borderRadius: 14, padding: 16, maxWidth: 400 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 7 }}>
            <div style={{ width: 30, height: 30, borderRadius: 9, background: p.primary, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Icon name="zap" size={16} color="#fff" /></div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => ctx.openGate(null)} style={{ background: p.primary, color: '#fff', border: 'none', padding: '9px 16px', borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>{t('btn.signup')}</button>
            ...
            <div key={m.id} style={{ marginBottom: 18, animation: 'rtFade .35s ease both' }}>
              {m.role === 'user' ? (
                <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                  <div style={{ maxWidth: '84%', background: p.userBub, color: '#fff', padding: '11px 16px', borderRadius: '18px 18px 5px 18px', fontSize: 14.5, lineHeight: 1.5, boxShadow: p.shadow }}>{m.text}</div>
                </div>
              ) : (
                <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    {m.text ? (
                      <div style={{ background: p.asstBub, border: `1px solid ${p.border}`, color: p.text, padding: '12px 16px', borderRadius: '5px 18px 18px 18px', fontSize: 14.5, lineHeight: 1.55, boxShadow: p.shadow }}>{m.text}</div>
                    ) : null}
    ...
    <div style={{ flex: 'none', borderTop: `1px solid ${p.border}`, background: p.header, padding: '12px 16px 14px' }}>
      <div style={{ maxWidth: 620, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, background: p.raised, border: `1px solid ${p.border2}`, borderRadius: 16, padding: '5px 5px 5px 15px', boxShadow: p.shadow }}>
          ...
          <button onClick={send} aria-label={t('btn.send')} style={{ flex: 'none', width: 40, height: 40, borderRadius: 12, border: 'none', background: p.primary, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', minHeight: 0, background: p.bg }}>
      ...
      <div style={{ flex: 'none', background: p.header, borderTop: `1px solid ${p.border}`, padding: '12px 16px', display: 'flex', gap: 10 }}>
        <button onClick={() => openGate(card)} style={{ flex: 1, background: p.primary, color: '#fff', border: 'none', padding: 13, borderRadius: 12, fontSize: 14.5, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
          <Icon name="message" size={18} color="#fff" /> {t('btn.contact')}
        </button>
      </div>
    </div>
  );
      ...
      <div className="rt-scroll" style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
        {favCount === 0 ? (
          <div style={{ height: '80%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '40px 24px' }}>
            ...
            <button onClick={newChat} style={{ background: p.primary, color: '#fff', border: 'none', padding: '11px 20px', borderRadius: 12, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>{t('btn.start_search')}</button>
          </div>
        ) : (
...
function ConversationsScreen({ ctx }: { ctx: Ctx }) {
  const { p, t, messages, newChat, back } = ctx;
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', minHeight: 0, background: p.bg }}>
      ...
      <div className="rt-scroll" style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
        <div style={{ maxWidth: 640, margin: '0 auto', padding: '18px 16px 30px' }}>
          ...
          {firstUser ? (
            <button onClick={back} style={{ textAlign: 'left', width: '100%', display: 'flex', alignItems: 'center', gap: 13, background: p.raised, border: `1px solid ${p.border}`, padding: '14px 15px', borderRadius: 14, cursor: 'pointer', boxShadow: p.shadow }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 600, color: p.text, fontSize: 14, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{firstUser.text}</div>
                <div style={{ fontSize: 12, color: p.muted, marginTop: 3 }}>{t('convos.current')}</div>
              </div>
...
function ContactGate({ ctx, phase, listing, onClose, onEnterChat }: { ctx: Ctx; phase: 'gate' | 'chat'; listing: ListingCard | null; onClose: () => void; onEnterChat: () => void }) {
  const { p, t } = ctx;
        ...
        {phase === 'gate' ? (
          <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <button onClick={onEnterChat} style={{ width: '100%', background: p.primary, color: '#fff', border: 'none', padding: 14, borderRadius: 13, fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>{t('btn.signup')}</button>
              <button onClick={onEnterChat} style={{ width: '100%', background: p.raised, color: p.text, border: `1px solid ${p.border2}`, padding: 14, borderRadius: 13, fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>{t('btn.login')}</button>
            </div>
          </>
        ) : (
          <>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: p.bg, border: `1px solid ${p.border2}`, borderRadius: 14, padding: '5px 5px 5px 14px', opacity: 0.85 }}>
              ...
              <div style={{ flex: 'none', width: 38, height: 38, borderRadius: 11, background: p.primary, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Icon name="send" size={17} color="#fff" /></div>
            </div>
...
export default function AssistantApp({ initialLanguage, initialTheme, theme: controlledTheme, transport }: AssistantAppProps = {}) {
  ...
  const { messages, isStreaming, currentTool, sendMessage, reset } = useChatStream(language, transport);
  ...
  const ctx: Ctx = {
    p, lang: language, t, toggleLang, setLang: setLanguage,
    ...
    back: () => setView('chat'),
    openGate: (c) => { setGateListing(c); setGatePhase('gate'); setGateOpen(true); },
  };
  ...
  return (
    <div style={{ ...(vars as unknown as CSSProperties), minHeight: '100vh', background: p.pageBg, display: 'flex', flexDirection: 'column', alignItems: 'center', fontFamily: "'Inter',system-ui,sans-serif", padding: '0' }}>
      <style>{KEYFRAMES}</style>
      <div style={{ position: 'relative', width: '100%', maxWidth: 1180, height: '100vh', background: p.bg, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <Header ctx={ctx} />
